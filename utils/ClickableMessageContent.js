...
class ClickableMessageContent {
  ...
  parseMessage() {
    try {
      ...
      const clickEventMatch = this.messageStr.match(
        /clickEvent=ClickEvent\{action=([^,]+),\s*value='([^']+)'\}/
      );
      if (clickEventMatch && clickEventMatch[1] === "RUN_COMMAND") {
        ...
        const hoverMatch = this.messageStr.match(
          /hoverEvent=HoverEvent\{[^}]*value='([^']+)'\}/
        );
  ...
  hasClickableCommand() {
    return (
      this.clickableContent !== null && this.clickableContent.command !== null
    );
  }
