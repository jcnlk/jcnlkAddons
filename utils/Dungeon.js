...
export function getIsInBoss(boss) {
  ...
  return bossName.removeFormatting().toLowerCase().includes(boss.toLowerCase());
}

// To track the current Goldor phase (S1, S2, S3, S4)
let currentGoldorPhase = 0;

const goldorTracker = new Feature("Catacombs").register("chat", (event) => {
  const msg = ChatLib.getChatMessage(event);
  if (msg === "[BOSS] Storm: I should have known that I stood no chance.") currentGoldorPhase = 1;
  if ((msg.includes("(7/7)") || msg.includes("(8/8)")) && !msg.includes(":")) currentGoldorPhase += 1;
});

goldorTracker.register("worldUnload", () => currentGoldorPhase = 0);
