...
// Tracks current goldor stage (S1, S2, S3, S4)
let currentStage = 0;

register("chat", (msg) => {
  if (!Dungeon.inBoss()) return;
  if (msg === "[BOSS] Storm: I should have known that I stood no chance.") currentStage = 1;
  if ((msg.includes("(7/7)") || msg.includes("(8/8)")) && !msg.includes(":")) currentStage += 1;
}).setCriteria("${msg}");
...
export const inFloor = (floor) => Location.inArea(`The Catacombs (${floor})`);
...
export const positionDefinitions = [
  ...
  },
  {
    id: "AtCore",
    messageText: "At Core!",
    checkCondition: () => inStage(2) || inStage(3),
