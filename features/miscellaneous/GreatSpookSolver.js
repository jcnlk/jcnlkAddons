...
function handlePublicSpeaking(playerName) {
  ...
  showDebugMessage(
    `Public Speaking started at: ${publicSpeakingStartTime}`,
    "info"
  );
  ...
  if (currentTime - lastPublicSpeakingTrigger < 5000) {
    showDebugMessage(
      "Public Speaking Demon triggered too quickly. Ignoring.",
      "info"
    );
    return;
  }
  ...
  showGeneralJAMessage(
    `Public Speaking Demon activated! Say something in chat (not too short)!`
  );
...
function handleCommitmentPhobia(message) {
  ...
  showDebugMessage(
    `Commitment Phobia started at: ${commitmentPhobiaStartTime}`,
    "info"
  );
  ...
  try {
    if (!message) {
      showDebugMessage(
        "Received null message in handleCommitmentPhobia",
        "warning"
      );
      return;
    }
    ...
    if (messageContent.hasClickableCommand()) {
      ...
      if (!command) {
        showDebugMessage(
          "No valid command found in clickable message",
          "warning"
        );
        return;
      }
      ...
      showDebugMessage(
        `Detected Commitment Phobia command: ${command}`,
        "info"
      );
...
register("chat", (player, time) => {
  if (player === Player.getName() && !mathSolved && mathStartTime) {
    ...
    showGeneralJAMessage(
      `Sucessfully solved Math Teacher in ${formatTime(solveTime)}!`
    );
    mathSolved = true;
    mathStartTime = null;
  }
...
register("chat", () => {
  if (publicSpeakingActive && publicSpeakingStartTime) {
    ...
    showGeneralJAMessage(
      `Successfully solved Public Speaking Demon in ${formatTime(solveTime)}!`
    );
    publicSpeakingActive = false;
    publicSpeakingStartTime = null;
  }
}).setChatCriteria(
  "[FEAR] Public Speaking Demon: Woohoo! Thank you for speaking out loud!"
);

register("chat", () => {
  if (publicSpeakingActive) {
    showGeneralJAMessage(
      "Your message was too short. Try saying something longer!"
    );
  }
...
register("chat", (message) => {
  if (commitmentPhobiaActive && commitmentPhobiaStartTime) {
    ...
    showGeneralJAMessage(
      `Successfully solved Commitment Phobia in ${formatTime(solveTime)}!`
    );
...
register("command", () => {
  if (lastCalculatedResult !== null && !mathSolved) {
    ...
    showDebugMessage(
      `Sent Great Spook answer: ${lastCalculatedResult}`,
      "info"
    );
  } else if (mathSolved) {
