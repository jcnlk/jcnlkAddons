...
function getCategorySymbol(category) {
    switch (category) {
        case "green": return "§a++§r";
        case "yellow": return "§e+§r";
        case "red": return "§c-§r";
        default: return "§7-§r";
    }
}
...
function scanChestContents(container) {
    if (!Config.enableDungeonChestHighlighting) {
        showDebugMessage("Dungeon chest highlighting is disabled, skipping scan", 'info');
        return;
    }
    ...
    if (!VALID_CHEST_TYPES.some(chestType => containerName.includes(chestType))) {
        showDebugMessage(`Container "${containerName}" is not a valid dungeon chest, skipping scan`, 'info');
        return;
    }
    ...
    showDebugMessage(`Container size: ${containerSize}, Slots to scan: ${slotsToScan}`, 'info');
    ...
    for (let i = 0; i < slotsToScan; i++) {
        ...
        if (item) {
            ...
            try {
                ...
                const itemName = item.getName().replace(/§./g, '');  // Remove color codes
                ...
                if (itemCategory || itemId === "ENCHANTED_BOOK") {
                    ...
                    if (itemId === "ENCHANTED_BOOK") {
                        const enchantDetails = getEnchantBookDetail(item);
                        if (enchantDetails) {
                            const enchantmentCategory = getEnchantmentCategory(`${enchantDetails.name} ${enchantDetails.level}`);
                            ...
                            showDebugMessage(`Enchanted Book found - Name: ${enchantDetails.name}, Level: ${enchantDetails.level}`, 'info');
                        } else {
                            outputString += ` (§cUnable to read enchantment)`;
                            showDebugMessage(`Unable to read enchantment details for book in slot ${i}`, 'warning');
                        }
                    }
                    ...
                    if (category) {
                        ...
                        itemsToHighlight.push({
                            slot: i,
                            color: category === "green" ? Renderer.color(0, 255, 0, 128) :
                                   category === "yellow" ? Renderer.color(255, 255, 0, 128) :
                                   Renderer.color(255, 0, 0, 128)
                        });
                    }
                }

                showDebugMessage(`Item processed - Name: ${itemName}, ID: ${itemId}`, 'info');
            } catch (error) {
                showDebugMessage(`Error processing item in slot ${i}: ${error.message}`, 'error');
            }
        } else {
